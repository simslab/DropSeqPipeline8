...
def pearclipper(fastq1_INFILE,fastq2_INFILE,pearaddress_OUTFILE,citeseq_INFILE,technology): # gzip compressed input and output
	...
	if technology == 'PearSeq':
		...
		offset=0
		N = 'NNNNNNNNNNNN'
	elif technology == 'CiteSeq3v3':
		bclen = 28
		cbclen = 16
		offset=0
		N = 'NNNNNNNNNNNNNNNN'
	elif technology == 'CiteSeq5v2':
		bclen=26
		cbclen=16
		offset=10
		N = 'NNNNNNNNNNNNNNNN'
	elif technology == 'CiteSeqTSB':
		bclen=28
		cbclen=16
		offset=10
		N = 'NNNNNNNNNNNNNNNN'
	...
	ct1=0
	ct2=0
	print(len(bc_seqs),sum([1 for s in bc_seqs if s.find('N')>-1]))
