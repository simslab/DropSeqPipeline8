...
def parse_user_input():
    ...
    parser.add_argument('-r1','--read1fastq',required=True,help='Path to read 1 fastq.')
    parser.add_argument('-r2','--read2fastq',required=True,help='Path to read 2 fastq.')
    parser.add_argument('-r','--refdir',required=True,help='Path to reference genome directory.')
    parser.add_argument('-g','--gtf',required=True,help='Path to gtf.')
    parser.add_argument('-h','--overhang',type=int,required=True,help='sjdbOverhang parameter for STAR.')
...
outdir = ui.outdir
prefix = ui.prefix
r1fastq_INFILE = ui.read1fastq
r2fastq_INFILE = ui.read2fastq
...
if technology != 'PearSeq':
    ...
    cmd = 'samtools view %(sortedbam_OUTFILE)s | python address.py %(gtf)s %(technology)s | gzip > %(address_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)
    ...
    pearcts_OUTFILE = outdir+'/'+prefix+'.pear_address.cts.txt'
    cmd = 'zcat %(pearaddress_INFILE)s.gz | python addressct.py %(tmp_OUTFILE)s %(pearcts_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)
    ...
    pearaddresscccts_OUTFILE = outdir+'/'+prefix+'.pear_address.cccts.txt'
    cmd = 'cat %(pearaddressccts_OUTFILE)s | python addressct2.py %(pearaddresscccts_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)

    pearfiltcts_OUTFILE = outdir+'/'+prefix+'.pear_address.filt.txt'
    cmd = 'cat %(pearaddresscccts_OUTFILE)s | python callfilter.py %(pearfiltcts_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)

    cmd = 'rm %(pearaddresscts_OUTFILE)s %(pearaddressccts_OUTFILE)s %(pearaddresscccts_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)
    
    pearaddresscts2_OUTFILE = outdir+'/'+prefix+'.pear_address.cts2.txt'
    pear_trackbcs_OUTFILE = outdir+'/'+prefix+'.pear_address.trackbcs.txt'
    cmd = 'python collapse2.py %(pearfiltcts_OUTFILE)s %(pearaddresscts2_OUTFILE)s 20 0.75 %(pear_trackbcs1_OUTFILE)s %(pear_trackbcs_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)

    cmd = 'rm %(pear_trackbcs1_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)

    pearaddressccts2_OUTFILE = outdir+'/'+prefix+'.pear_address.ccts2.txt'
    cmd = 'cat %(pearaddresscts2_OUTFILE)s | python addressct2.py %(pearaddressccts2_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)

    pearcfilt_OUTFILE = outdir+'/'+prefix+'.pear_address.cfilt.txt'
    cmd = 'cat %(pearaddressccts2_OUTFILE)s | python callfilter.py %(pearcfilt_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)

    cmd = 'rm %(pearaddresscts2_OUTFILE)s %(pearaddressccts2_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)

    pearcfiltpdf_OUTFILE = outdir+'/'+prefix+'.pear_address.cfilt.pdf'
    pearcfiltchist_OUTFILE = outdir+'/'+prefix+'.pear_address.cfilt.chist.txt'
    cmd = 'python dsstats.py %(pearcfilt_OUTFILE)s %(pearcfiltpdf_OUTFILE)s %(pearcfiltchist_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)
