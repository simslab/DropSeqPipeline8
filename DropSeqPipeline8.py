...
if technology == 'PearSeq':
    if not ui.citeseq_barcodes:
        ...
        cmd = 'zcat %(r2fastq_INFILE)s | python callclipper.py %(r1fastq_INFILE)s PearSeq %(pearaddress_INFILE)s %(citeseq_INFILE)s | gzip > %(r2fastqclip_INFILE)s' % vars()
        print(cmd)
        os.system(cmd)
...
if technology == 'PearSeq':
    tmpfile = outdir+'/tmp.gz'
    cmd = 'zcat %(address_OUTFILE)s %(pearaddress_INFILE)s.gz > %(tmpfile)s' % vars()
    print(cmd)
    os.system(cmd)
    cmd = 'mv %(tmpfile)s %(address_OUTFILE)s' % vars()
    print(cmd)
    os.system(cmd)
